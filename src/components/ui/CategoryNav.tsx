        ...
        <>
            ...
            {deviceType === 'mobile' && (
                <>
                    ...
                    >
                        <span
                            className={`w-5 h-0.5 bg-white transition-all ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
                    ...
                    {/* 현재 번호 표시 */}
                    <div className="fixed top-6 left-6 z-50 text-white font-bold text-sm">
                        {currentNumber}
                    </div>
                    ...
                    {isMenuOpen && (
                        <>
                            ...
                            {/* 메뉴 컨텐츠 */}
                            <nav
                                className="fixed top-0 right-0 w-64 h-screen bg-black/90 backdrop-blur-xl border-l border-white/10 z-40 flex flex-col items-start justify-center gap-6 px-8">
                                {categories.map((category) => (
                                    <button
                                        key={category.id}
                                    ...
                                    >
                                        ...
                                        {currentCategory === category.id && (
                                            <div
                                                className="absolute -left-3 top-1/2 -translate-y-1/2 w-1 h-8 bg-purple-400 rounded-full"/>
                                        )}
                                    </button>
                                ))}
                            </nav>
