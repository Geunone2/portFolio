...
import { TEXTURE_MAPPING, type TextureMappingKey } from './textureConfig';
import type { ModelLoadResult } from '../types';
import { loadHighQualityTexture } from './loadHighQualityTexture';
                        ...
                        if (config) {
                            if (config.texturePath) {
                                ...
                                const texturePromise = loadHighQualityTexture(textureLoader, config.texturePath)
                                    .then((newTexture) => {
                                        meshesWithTextures.push({ mesh: child, material, texture: newTexture });
                                    });
                ...
                try {
                    ...
                    await Promise.all(
                        textureLoadPromises.map(async (promise) => {
                            ...
                            loadedCount++;
                            if (onProgress) {
                                onProgress((loadedCount / totalTextures) * 100);
                            }
                        })
                    );
